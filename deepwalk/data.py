...
def preprocess(property_path,house_path,street_path,region_path):
    ...
    ten_feats= torch.cat([out_feats['house'],out_feats['property'],out_feats['region'],out_feats['street']], dim=0)
    ...
    G.nodes['property'].data['hv']=out_feats['property']
    G.nodes['house'].data['hv'] = out_feats['house']
    G.nodes['street'].data['hv'] = out_feats['street']
    G.nodes['region'].data['hv'] = out_feats['region']
    house_labels, street_labels, region_labels = get_price()
    property_labels = torch.tensor(df_property['price'], dtype=torch.float) / 1000000
    homogeneous_graph = dgl.to_homogeneous(G, ndata=['hv'])

    return homogeneous_graph,property_labels,house_labels, street_labels, region_labels,ten_feats
